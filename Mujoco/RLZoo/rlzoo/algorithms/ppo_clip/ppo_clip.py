...
class PPO_CLIP(object):
    ...
    def __init__(self, net_list, optimizers_list, epsilon=0.2, lle=False, K=0,
                 Phi_gradient_steps=1, lle_lr=1e-3, lle_batch_size=1):
        """
        ...
        self.lle_lr = lle_lr
        self.lle_batch_size = lle_batch_size

        if lle:
            self.critic, self.actor, self.lle_net = net_list
        else:
    ...
    def update(self, frame_idx, s, a, r, a_update_steps, c_update_steps):
        """
            ...
            for ep in range(1, train_episodes + 1):
                ...
                for t in range(max_steps):  # in one episode
                    ...
                    if (t + 1) % batch_size == 0 or t == max_steps - 1 or done:
                        ...
                        self.update(frame_idx, bs, ba, br, a_update_steps, c_update_steps)
                    if done:
                        break
