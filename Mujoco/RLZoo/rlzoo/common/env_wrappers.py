...
from gym.wrappers import FlattenObservation, FilterObservation, GrayScaleObservation  # FlattenDictWrapper
...
def build_env(env_id, env_type, vectorized=False,
    ...
    stack = env_type == 'atari' or env_type == 'dm_control-images'
    if nenv > 1:
        if vectorized:
...
def _make_env(env_id, env_type, seed, reward_shaping, frame_stack, **kwargs):
    ...
    if env_type == 'atari':
        ...
        env = DmObsTrans(env)
    elif env_type == 'dm_control_images':
        action_repeat = 1
        frame_stack = 4
        image_size = 84
