...
def plot(episode_rewards, algorithm_name, env_name, ver='0.0-0'):
    """
    ...
    """
    path = os.path.join('./Results/Results-{}'.format(ver), 'img')
...
def plot_save_log(episode_rewards, algorithm_name, env_name, ver='0.0'):
    """
    plot the learning curve, saved as ./img/algorithm_name-env_name.png,
    ...
    :param env_name: string
    """
    path = os.path.join('./Results/Results-{}'.format(ver), 'log')
    name = algorithm_name + '-' + env_name
    plot(episode_rewards, algorithm_name, env_name, ver)
    if not os.path.exists(path):
        os.makedirs(path)
    np.save(os.path.join(path, name), episode_rewards)


def save_model(model, model_name, algorithm_name, env_name, ver='0.0'):
    """
    ...
    """
    name = algorithm_name + '-' + env_name
    path = os.path.join('./Results/Results-{}'.format(ver), 'model', name)
    if not os.path.exists(path):
        os.makedirs(path)
    tl.files.save_npz(model.trainable_weights, os.path.join(path, model_name))


def load_model(model, model_name, algorithm_name, env_name, ver='0.0'):
    """
...
def call_default_params(env, envtype, alg, lle, lle_params, default_seed=True):
    """ Get the default parameters for training from the default script """
    alg = alg.lower()
    default = import_module('.'.join(['rlzoo', 'algorithms', alg, 'default']))
    params = getattr(default, envtype)(env, lle, lle_params, default_seed)  # need manually set seed in the main script if default_seed = False
    return params
