...
class LCRNetwork():
    def __init__(self, input_dim, K, name=None):
        """
        LCR Network to output linear combination of nearest features
        Args:
            input_dim: (gym.spaces) space of the state from gym environments
            K: number of nearest features
        """
        input_layer = tf.keras.Input(shape=(input_dim, K))

        with tf.name_scope('Output'):
            '''Constrain to positive weights to eliminate over fitting'''
            self.outputs = tf.keras.layers.Dense(1, use_bias=False, name='lcr_outputs',
                                            kernel_constraint=tf.keras.constraints.NonNeg())(input_layer)

        self.input_dim = input_dim
        self.model = tf.keras.Model(inputs=input_layer, outputs=self.outputs)
...
class QNetwork(Model):
    def __init__(self, state_space, action_space, hidden_dim_list,
        ...
        if isinstance(state_space, spaces.Dict):
            ...
            if self.state_only:
                ...
                super().__init__(inputs=[obs_inputs, act_inputs], outputs=[outputs, rep_layer], name=name)
        if trainable:
            self.train()
        else:
            self.eval()
    ...
    def nearest(self, inputs, *args, **kwargs):
        if self.state_only:
            states = inputs
        else:
            states, actions = inputs
